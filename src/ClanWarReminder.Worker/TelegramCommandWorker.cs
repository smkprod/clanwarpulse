...
public sealed class TelegramCommandWorker : BackgroundService
{
    ...
    {
        if (!HasUsableBotToken(_telegramOptions.BotToken))
        {
            _logger.LogWarning("TelegramCommandWorker disabled because Telegram bot token is not configured.");
            return;
        }

        var http = _httpClientFactory.CreateClient();
        await EnsurePollingReadyAsync(http, _telegramOptions.BotToken, stoppingToken);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Telegram command polling failed.");
    ...
    private async Task HandleMessageAsync(TelegramMessageDto message, CancellationToken cancellationToken)
    {
        ...
        var command = rawCommand.Split('@', 2)[0].ToLowerInvariant();
        ...
        CancellationToken cancellationToken)
    {
        ...
        var group = await setupService.SetupAsync(PlatformType.Telegram, chatId, clanTag, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var played = status.Played.Count;
            var notPlayed = status.NotPlayed.Count;
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var platformUserId = message.From.Id.ToString();
            var displayName = BuildUserDisplayName(message.From);
            var tag = args[0];
